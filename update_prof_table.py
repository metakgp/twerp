...
def create_prof_page(prof_data,site,timetable):
    name=prof_data['name']
    split=name.split()
    join_with_underscore='_'.join(split)
    link='https://wiki.metakgp.org/w/{}'.format(join_with_underscore)
    ...
    msg = "Create Page for {}".format(prof_data['name'])
    prof_page.save(msg)
    print("Created page for {} with link {}".format(prof_data['name'],link))
